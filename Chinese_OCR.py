...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        for i in range(len(final_reco_text)):
            inference_result += final_reco_text[i]
        print(inference_result)
        print('=======================================================')
        # 识别程序计时结束
        time_end = time.time()
        print('\n识别程序运行耗时{}s'.format(time_end - time_start))
        with open(r'D:\py\SZX-OCR-Engine\ocr\OCR_RESULT.txt', 'w', encoding='utf-8') as file:
            file.write(inference_result)
